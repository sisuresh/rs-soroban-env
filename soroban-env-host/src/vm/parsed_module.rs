...
impl ParsedModule {
    ...
    fn parse_wasm<Ctx: CompilationContext>(
        ...
        // The wasmparser-based pre-validation below was added in p25 to pin
        // down the error code we report for malformed modules (so it doesn't
        // drift with wasmi internals). Skipping it under p23/p24 keeps replay
        // byte-identical with hosts that didn't have this pre-check: wasmi
        // still validates the module below, so the accept/reject decision is
        // unchanged.
        if curr_ledger_protocol >= 25 {
