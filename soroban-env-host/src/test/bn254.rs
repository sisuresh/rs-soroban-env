...
fn compressed_g1(host: &Host, rng: &mut StdRng) -> Result<BytesObject, HostError> {
    let mut buf: Vec<u8> = Vec::with_capacity(G1_SERIALIZED_SIZE / 2);
...
fn bn254_g2_affine_serialize_uncompressed(
    ...
    host.bn254_serialize_uncompressed_into_slice::<G2_SERIALIZED_SIZE, _>(g2, &mut buf, "G2")?;
    host.add_host_object(host.scbytes_from_slice(&buf)?)
}
...
fn test_bn254_serialization_roundtrip() -> Result<(), HostError> {
    ...
    let g2_deser = host.bn254_affine_deserialize::<G2_SERIALIZED_SIZE, _>(bytes, true, "G2")?;
...
fn test_bn254_multi_pairing_check() -> Result<(), HostError> {
    ...
    // 5. Basic pairing bilinearity test: e(P, Q) should be consistent
    {
        let p = sample_g1(&host, &mut rng)?;
        let q = sample_g2(&host, &mut rng)?;

        // Test same pairing twice for consistency
        let vp1 = host.vec_new_from_slice(&[p.to_val()])?;
        let vp2 = host.vec_new_from_slice(&[q.to_val()])?;
        let result1 = host.bn254_multi_pairing_check(vp1, vp2)?;

        let vp1_2 = host.vec_new_from_slice(&[p.to_val()])?;
        let vp2_2 = host.vec_new_from_slice(&[q.to_val()])?;
        let result2 = host.bn254_multi_pairing_check(vp1_2, vp2_2)?;

        // Results should be consistent
        assert_eq!(
            result1.to_val().get_payload(),
            result2.to_val().get_payload()
        );
    }
